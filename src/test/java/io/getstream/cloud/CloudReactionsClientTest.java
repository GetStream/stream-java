...
public class CloudReactionsClientTest {
  ...
  public void get() throws Exception {
    ...
    Reaction data = Reaction.builder().activityID("ed2837a6-0a3b-4679-adc1-778a1704852d").kind("like").build();
  ...
  public void filter() throws Exception {
    ...
    Activity activity = client.flatFeed("flat", "reactor")
        .addActivity(Activity.builder().actor("this").verb("done").object("that").build()).join();

    client.reactions().add("john-doe", "like", activity.getID()).join();

    client.reactions().filter(LookupKind.ACTIVITY, activity.getID()).join();

    List<Reaction> result = client.reactions().filter(LookupKind.ACTIVITY_WITH_DATA, activity.getID(), "comment")
        .join();
  }
  ...
  public void update() throws Exception {
    ...
    Reaction data = Reaction.builder().id("b5c46f9b-0839-4207-86aa-6a7f388b7748").kind("like")
        .extraField("key", "value").build();
