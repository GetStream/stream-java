...
public final class CollectionsClient {
  ...
  public <T> CompletableFuture<T> addCustom(String userID, String collection, T item)
      throws StreamException {
    return add(userID, collection, convert(item, CollectionData.class))
        .thenApply(data -> convert(data, (Class<T>) item.getClass()));
  }

  public CompletableFuture<CollectionData> add(String collection, CollectionData item)
      throws StreamException {
  ...
  public <T> CompletableFuture<T> updateCustom(String userID, String collection, T item)
      throws StreamException {
    return update(userID, collection, convert(item, CollectionData.class))
        .thenApply(data -> convert(data, (Class<T>) item.getClass()));
  }

  public CompletableFuture<CollectionData> update(String collection, CollectionData item)
      throws StreamException {
  ...
  public <T> CompletableFuture<Void> upsertCustom(String collection, Iterable<T> items)
      throws StreamException {
  ...
  public <T> CompletableFuture<Void> upsertCustom(String collection, T... items)
      throws StreamException {
    final CollectionData[] custom =
  ...
  public CompletableFuture<Void> upsert(String collection, Iterable<CollectionData> items)
      throws StreamException {
    return upsert(collection, Iterables.toArray(items, CollectionData.class));
  }

  public CompletableFuture<Void> upsert(String collection, CollectionData... items)
      throws StreamException {
    final Token token = buildCollectionsToken(secret, TokenAction.WRITE);
    return collections.upsert(token, collection, items);
  }

  public <T> CompletableFuture<T> getCustom(Class<T> type, String collection, String id)
      throws StreamException {
    return get(collection, id).thenApply(data -> convert(data, type));
  }

  public CompletableFuture<CollectionData> get(String collection, String id)
      throws StreamException {
    final Token token = buildCollectionsToken(secret, TokenAction.READ);
    return collections.get(token, collection, id);
  }

  public <T> CompletableFuture<List<T>> selectCustom(
      Class<T> type, String collection, Iterable<String> ids) throws StreamException {
    return selectCustom(type, collection, Iterables.toArray(ids, String.class));
  }

  public <T> CompletableFuture<List<T>> selectCustom(
      Class<T> type, String collection, String... ids) throws StreamException {
  ...
  public CompletableFuture<List<CollectionData>> select(String collection, Iterable<String> ids)
      throws StreamException {
    return select(collection, Iterables.toArray(ids, String.class));
  }

  public CompletableFuture<List<CollectionData>> select(String collection, String... ids)
      throws StreamException {
    final Token token = buildCollectionsToken(secret, TokenAction.READ);
  ...
  public CompletableFuture<Void> deleteMany(String collection, Iterable<String> ids)
      throws StreamException {
    return deleteMany(collection, Iterables.toArray(ids, String.class));
  }

  public CompletableFuture<Void> deleteMany(String collection, String... ids)
      throws StreamException {
    final Token token = buildCollectionsToken(secret, TokenAction.DELETE);
